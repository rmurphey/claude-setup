...
export class GitHubAPI {
  ...
  private getCurrentRepo(): string {
    try {
      ...
      throw new GitHubAPIError(
        'Could not determine GitHub repository. Make sure you\'re in a git repository with a GitHub remote.',
        { endpoint: 'git remote' }
      );
    }
  }
   ...
   */
  async fetchIssue(issueNumber: number, repoOverride: string | null = null): Promise<Result<GitHubIssue, GitHubAPIError>> {
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error: new GitHubAPIError(
            ...
            { status: response.status, endpoint: url, repo }
          )
        };
      }
   ...
   */
  async listIssues(
    filters: GitHubListIssuesFilters = {}, 
    repoOverride: string | null = null
  ): Promise<Result<GitHubIssue[], GitHubAPIError>> {
    const repo = repoOverride ?? this.repo;
   ...
   */
  private getIssueType(issue: GitHubIssue): string {
   ...
   */
  async createIssueBranch(issue: GitHubIssue): Promise<Result<string, GitHubAPIError>> {
   ...
   */
  async createIssueContext(issue: GitHubIssue): Promise<Result<string, GitHubAPIError>> {
