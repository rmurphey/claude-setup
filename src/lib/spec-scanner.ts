...
import { SpecScanner, SpecValidationResult, ValidationError } from '../types/archival.js';
 ...
 */
export class SpecScannerImpl implements SpecScanner {
  ...
  private completionDetector: SpecCompletionDetectorImpl;
   ...
   */
  async getIncompleteSpecs(): Promise<string[]> {
    const allSpecs = await this.getAllSpecs();
   ...
   */
  async validateSpec(specPath: string): Promise<SpecValidationResult> {
    const issues: string[] = [];
   ...
   */
  async scanAndValidateAllSpecs(): Promise<{
    totalSpecs: number;
    validSpecs: string[];
    invalidSpecs: string[];
   ...
   */
  private async isSpecDirectory(dirPath: string): Promise<boolean> {
    try {
   ...
   */
  private async validateTasksFile(specPath: string, issues: string[], warnings: string[]): Promise<void> {
    try {
   ...
   * @param warnings Warnings array to append to
   */
  private async checkForUnexpectedFiles(specPath: string, warnings: string[]): Promise<void> {
    try {
   ...
   * @param warnings Warnings array to append to
   */
  private async validateFileContent(specPath: string, issues: string[], warnings: string[]): Promise<void> {
