...
export class ArchivalEngineImpl {
    ...
    configManager;
    specScanner;
    constructor(archiveLocation = ArchivalEngineImpl.DEFAULT_ARCHIVE_LOCATION, configDirectory = '.') {
     ...
     */
    async shouldArchiveSpec(specPath) {
        const config = await this.getConfig();
     ...
     * @returns Promise<ArchivalResult> Result of the archival operation
     */
    async archiveSpecWithConfig(specPath) {
     ...
     */
    async autoArchiveCompletedSpecs() {
        const config = await this.getConfig();
