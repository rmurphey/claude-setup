...
export class CLIMain {
    ...
    validateFlagCombinations(config) {
        ...
        for (const flag of activeFlags) {
            ...
            if (conflicts) {
                const conflictingFlags = conflicts.filter(conflictFlag => activeFlags.includes(conflictFlag));
        ...
        for (const flag of activeFlags) {
            ...
            if (dependencies) {
                const missingDeps = dependencies.filter(depFlag => !activeFlags.includes(depFlag));
    ...
    async runSetupMode(config) {
        ...
        if (config.language) {
            await orchestrator.runSetupMode({ languageOverride: config.language });
        }
        else {
            await orchestrator.runSetupMode();
        }
    }
    /**
