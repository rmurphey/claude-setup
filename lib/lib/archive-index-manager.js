...
export class ArchiveIndexManager {
    ...
    validateAndMigrateIndex(data) {
        ...
        if (!data.version || data.version !== ArchiveIndexManager.INDEX_VERSION) {
            // For now, just update version if missing or different
            data.version = ArchiveIndexManager.INDEX_VERSION;
        }
        ...
        data.archives = data.archives.map((entry) => {
            ...
            return {
                ...
                completionDate: entry.completionDate ? new Date(entry.completionDate) : new Date(),
                archivalDate: entry.archivalDate ? new Date(entry.archivalDate) : new Date(),
