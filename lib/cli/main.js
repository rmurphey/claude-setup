...
import { runRecovery } from '../recovery-system.js';
...
export class CLIMain {
   ...
   * Run recovery mode
   */
  async runRecoveryMode(_config) {
    console.log(chalk.blue.bold('\n🔧 Claude Setup Recovery\n'));
    await runRecovery(this.args);
  }

  /**
   * Run language detection mode
   */
  async runLanguageDetectionMode(_config) {
    console.log(chalk.blue.bold('\n🔍 Language Detection\n'));
   ...
   */
  async runConfigurationMode(_config) {
   ...
   */
  async runSyncIssuesMode(_config) {
  ...
  /**
   * Run interactive setup mode (default)
   */
  async runSetupMode(config) {
    ...
    // Pass language override if specified
    if (config.language) {
      await interactive.runInteractiveSetup({ languageOverride: config.language });
    } else {
